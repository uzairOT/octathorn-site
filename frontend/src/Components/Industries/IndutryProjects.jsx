...
function PortfolioProjects({ Heading, Discription }) {
    let { heading } = useParams();
    // let heading = 'healthcare'

    let conditionalHeading = heading ? heading : "healthcare" || "finance" || "defence" || "publicsector" || "manufacturing" || "telecom" || "ecommerce";
    ...
    useEffect(() => {
        axios.get('https://octathorn.com/api/project')
            .then(response => {
                ...
                const filteredData = response.data.filter(item => item.industry.trim().toLowerCase() === conditionalHeading.trim().toLowerCase());
    ...
    }, [conditionalHeading]); // Add heading to the dependency array
                                    ...
                                    <>
                                        ...
                                        <Grid item xs={12} sm={12} md={12} lg={6} mt={1}>
                                            ...
                                            <Card sx={{ ...themeStyles.contentCard, }}>
                                                <CardContent>
                                                    <Typography variant="h2" sx={themeStyles.cardtitle}>
                                                        {project.title}
                                                    </Typography>
                                                    <Typography variant="subtitle2" sx={themeStyles.carddescription}>
                                                        {project.description}
                                                    </Typography>
